...
/*
[AT] Increase maximum number of IRKs allowed.
#define MAX_IRKS 4
*/
#define MAX_IRKS 20

/* [AT] change count to a global variable. Otherwise existing IRKs already loaded
will be overwritten by newly loaded IRKs.
*/
...
static void cmd_irks(int argc, char **argv)
{
	...
	*/
	...
	/* [AT] count is global -- already reset during declaration. 
	count = 0;
	*/

	while ((opt = getopt_long(argc, argv, "+rl:f:h",
					irks_options, NULL)) != -1) {
		switch (opt) {
		case 'l':
			if (count >= MAX_IRKS) {
		...
		case 'f':
			...
			break;
		case 'r':
		/* [AT] add option -r to reset the counter; effectively removing the IRKs from
			the kernel. */
			count=0;
			print("Clearing IRK list"); 
			break;
		case 'h':
			bt_shell_usage();
			optind = 0;
...
static void register_mgmt_callbacks(struct mgmt *mgmt, uint16_t index)
{
	...
	mgmt_register(mgmt, MGMT_EV_DISCOVERING, index, discovering,
								NULL, NULL);
	/* [AT] register callback for new irk event */
	mgmt_register(mgmt, MGMT_EV_NEW_IRK	, index, new_irk,
								NULL, NULL);
...
static void cmd_eir_data(int argc, char **argv)
{
	uint8_t val; 
	parse_setting(argc, argv, &val);
	show_eir_data = (val == 1);
...
static const struct bt_shell_menu main_menu = {
	...
	{ "eir_data",	"<on/off>",
	cmd_eir_data,		"show/hide eir_data in scan result",	},
...
/*	
	[AT] for some reason this menu doesn't work. Keeps printing
	parsing error for optional argument. I think the parser 
	does not expect the syntax <file path> for an optional argument?
	{ "irks",		"[--local <index>] [--file <file path>]",
		cmd_irks,		"Load Identity Resolving Keys"	},
*/
	{ "irks",		"[-r] [-l index] [-f filepath]",
		cmd_irks,		"Load Identity Resolving Keys"	},
